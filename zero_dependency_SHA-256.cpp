...
To compare, do "sha256sum /your/file" */

#include <iostream>
#include <string>
int main()
{	std::string SHA_256_input = "abc"; //Can be any file too.
	...
	//Pads input.
	//Your input is processed 64 bytes at-a-time, first-to-last. So bytes are appended to your input, until it's a multiple of 64.
	...
	{	unsigned int W[64];
		...
		//Expands the 16 words into 64 words.
		for(int t = 16; t < 64; t++)
	...
	This block can be placed at the end of the big loop above, or anywhere, so you can watch the hash evolve over time.
	You can even copy the state of the hash at any point, and break out, so that you can finish creating the hash using
	the last step or few steps. That way, you can modify the end of the input and create the hash you want faster.   */
	SHA_256_output = "";
