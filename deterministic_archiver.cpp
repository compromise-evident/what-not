/*The world's first! Turn any folder into a REPRODUCIBLE                        Run it: "apt install g++ geany". Open the .cpp in Geany. Hit F9 once. F5 to run.
...
{	char file_byte;
	...
	{	//Gets path.
		...
		{	//Creates folder.
			...
			{	line.erase(0, 8); std::string dir = "unpacked/"; dir += line;
			...
			if(std::getline(in_stream, line)) {line.erase(0, 6); bytes_to_read = std::stoull(line);}
			...
			if(!in_stream.get(file_byte)) {std::cout << "error"; return 0;}
			out_stream.close();
		}
		in_stream.close();
	}
