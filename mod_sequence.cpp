...
{	unsigned long long wanted_remainder = 0;
	...
	{	for(unsigned long long divisor = 1;; divisor++)
		{	if((dividend % divisor) == wanted_remainder) {std::cout << wanted_remainder << " = " << dividend << " mod " << divisor << "\n"; break;}
			if (divisor  > max_divisor)                  {std::cout << wanted_remainder << " = " << dividend << " mod >max_divisor\n";      break;}
		}
	}
}
